...
public class Secret {
    ...
    private void translate(byte[] data, byte[] tranTable) {
        for (int i = 0; i < data.length; i++) { data[i] = tranTable[data[i] & 255]; }
    }
    ...
    private byte[] getTable(String keyString) {
        ...
        try {
            ...
            for (int i = 0; i < 8; i++) { bigInt[i + 1] = md5[7 - i]; }
        ...
        byte b = 0;
        for (int i = 0; i < 256; i++, b++) { table[i] = b; }
    
        for (salt = 1; salt < 1024; salt++) { mergeSort(table, new byte[256], 0, 255); }
    ...
    Secret(String keyString) {
        ...
        rtable = new byte[256];
        byte b = 0;
        for (int i = 0; i < 256; i++) { rtable[table[i] & 255] = b++; }
    }
